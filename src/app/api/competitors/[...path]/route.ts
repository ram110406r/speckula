// Generic proxy for /competitors/* backend routes.
// Supports GET with optional query string pass-through.
...
async function forward(req: Request, segments: string[]): Promise<Response> {
  ...
  const upstreamPath = `/competitors/${segments.join('/')}${url.search}`;
