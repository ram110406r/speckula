// Generic proxy for /notifications/* backend routes.
...
async function forward(req: Request, segments: string[]): Promise<Response> {
  ...
  // Notifications root (empty path) is valid for GET /notifications and DELETE /notifications.
  ...
  const upstreamPath = segments.length
    ? `/notifications/${segments.join('/')}${url.search}`
    : `/notifications${url.search}`;
