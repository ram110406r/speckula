// Generic proxy for /workspaces backend routes.
// Supports GET, POST, PATCH, DELETE with optional query string pass-through.
...
async function forward(req: Request, segments: string[]): Promise<Response> {
  ...
  const upstreamPath = `/workspaces${segments.length ? `/${segments.join('/')}` : ''}${url.search}`;
