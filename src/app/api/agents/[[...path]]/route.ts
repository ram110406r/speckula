// Generic proxy for /agents and /agents/* backend routes.
// Handles the base path (roster GET, create POST) and sub-paths
// (jobs, history, :id detail/patch/delete). Mirrors the experiments proxy.
...
async function forward(req: Request, segments: string[]): Promise<Response> {
  ...
  const upstreamPath = `/agents${segments.length ? `/${segments.join('/')}` : ''}${url.search}`;
