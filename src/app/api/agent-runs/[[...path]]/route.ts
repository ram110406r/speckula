// Generic proxy for /agent-runs and /agent-runs/* backend routes.
// Handles both the base path (list / create) and sub-paths (detail, step,
// complete, stop, stats). Mirrors the experiments proxy.
...
async function forward(req: Request, segments: string[]): Promise<Response> {
  ...
  const upstreamPath = `/agent-runs${segments.length ? `/${segments.join('/')}` : ''}${url.search}`;
