// Generic proxy for /product-brain/* backend routes.
// Supports GET, POST, DELETE with optional query string pass-through.
...
async function forward(req: Request, segments: string[]): Promise<Response> {
  ...
  const upstreamPath = `/product-brain/${segments.join('/')}${url.search}`;
